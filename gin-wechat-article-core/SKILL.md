...
1. 读取 `gin-wechat-article-core/VERSION`，输出触发反馈；主 skill 生成或读取 `article_id`；执行版本自检（读取本 skill 目录下 `.last-update-check`，距今超过 30 天则对比远程 origin HEAD，确认落后时在当前任务完成后提示用户更新，不自动执行）。
...
4. **调用 `scripts/style_selector.py` 进行风格选择与素材读取：**
   ...
   - 主 skill 读取以下参考文件，提取核心要点，生成 `reference_briefs` 并写入 `context.md`：
     - 大纲/角度类：`gin-wechat-article-core/references/angle-library.md`、`hook-design.md`、`content-outline-framework.md`
...
| angle_diagnosed | `template_loaded` 已完成 | `narrative_protocol` 已生成 | 无（AI 自动节点） | 重新运行 `gin-wechat-article-angle` | role_boundary |
...
- `angle_diagnosed`：由 `gin-wechat-article-angle` 完成素材诊断并推荐角度。
...
1. **核心子 skill 调用链不可变更**
   - `outline_selected` 阶段必须调用 `gin-wechat-article-writer` 写正文初稿。
   - `draft_revised` 阶段必须调用 `gin-wechat-article-polish` 做润色和小标题优化。
   - `title_confirmed` 阶段必须调用 `gin-wechat-article-quality` 做质量自检。
   - **禁止用 `human-writing` 或其他任何写作 skill 替代 `gin-wechat-article-writer` 或 `gin-wechat-article-polish`。**
...
requirements:                         # gin-wechat-article-clarify 写入
  topic: ""
  target_reader: ""
  core_points: []
  word_count: 2500
...
title_candidates:                     # gin-wechat-article-title 写入
  - rank: 1
    ...
    risk: "..."
selected_title: ""
quality_report: {}                    # gin-wechat-article-quality 写入
fixed_text: ""                        # gin-wechat-article-quality 自动修复后的文本
finalized: false
...
- `narrative_protocol`：由 `template_loader.extract_narrative_protocol()` 基于 `selected_template` 生成，包含 `derived_from`、`sections`、`global_rules`、`tone`、`forbidden_zone`，供 `gin-wechat-article-writer` / `gin-wechat-article-polish` 等写作子 skill 引用。
...
- `selected_angle` / `emotion_trigger` / `secondary_trigger`：由 gin-wechat-article-angle 选定并写入。
- `article_type`：文章类型，从 `selected_template.id` 映射，供 gin-wechat-article-quality 做 frontmatter 检查。
- `emotion_tone`：情绪基调，从风格模板 `情绪基调` 提取的关键词，供 gin-wechat-article-quality 做 frontmatter 检查。
- `angle_candidates`：gin-wechat-article-angle 生成的可用角度列表。
