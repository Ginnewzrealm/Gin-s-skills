...
| 3.4 | 自带四层自检，和 quality 重复 | `SKILL.md` 第四步 | 自己跑 L1-L4 检查 | 职责和 wechat-article-quality 重叠 |
...
| 文件 | 内容 | 消费方 |
|------|------|--------|
| `wechat-article-core/references/expansion-methodology.md` | 核心原则、材料优先、说话位置、内容骨架、中文句法纪律 | `wechat-article-outline`、`khazix-writer` |
| `wechat-article-core/references/ai-flavor-guide.md` | 四层 AI 味模型、六种 AI 味诊断、24 种 AI 写作模式 | `wechat-article-quality`、`wechat-article-polish` |
| `wechat-article-core/references/writing-checklist.md` | 通用禁用项、文采增强四机制、内容债/语言债诊断 | `khazix-writer`、`wechat-article-quality`、`wechat-article-polish` |
...
|---------|---------|----------|--------------|
| wechat-article-clarify | 需求确认问题模板（含说话位置三问） | 不再收集素材内容 | requirements 增加 `speaker_position`，增加素材完整性确认 |
| wechat-article-angle | `materials_full.md`、`narrative_protocol` | 不再依赖截断的字符串 summary | angle_candidates 增加与 narrative_protocol 的适配说明 |
| wechat-article-outline | `narrative_protocol`、`expansion-methodology.md`、`materials_full.md` | sections 不按 narrative_protocol 生成的问题 | sections 增加 `materials_ref` 和 `human_needed` |
| khazix-writer | `narrative_protocol`、`expansion-methodology.md`、`writing-checklist.md`、`writing-style.md`（精简版）、`outline.sections[].materials_ref` | 删除 AI 角色边界、删除四层自检、删除卡兹克个人风格绑定 | 按 section 规则扩写，只做轻量禁区扫描，标注 `ai_filled` / `needs_human_experience`，frontmatter 保留 `narrative_protocol_derived_from` |
| wechat-article-polish | `narrative_protocol`、`ai-flavor-guide.md`、`writing-checklist.md` | 无 | 润色时遵守 narrative_protocol 的 tone 和 forbidden_zone |
| wechat-article-title | `narrative_protocol.global_rules.opening` | 无 | 标题生成符合模板开头规则 |
| wechat-article-quality | `narrative_protocol.forbidden_zone`、`ai-flavor-guide.md`、`writing-checklist.md`（完整版） | 调整 L1-L4 与方法论对齐 | 增加内容债/语言债诊断、模板专属禁区检查，统一负责完整四层质检 |
...
outline_selected
  ↓
draft_written（khazix-writer 出初稿）
  ↓
draft_revised → 人工改写
  ↓
polished
...
> **说明**：由于上游已有专门整理素材的 skill，`wechat-article-clarify` 不再重复收集素材内容，只确认素材是否齐全、需求是否明确。
...
### 4.5 改 wechat-article-outline

#### 输入增加

- `context.md.narrative_protocol`
- `wechat-article-core/references/expansion-methodology.md`
- `materials_full.md`
...
`khazix-writer` 不再绑定卡兹克个人风格。风格由 YAML 模板和 `references/writing-style.md` 共同控制，本 skill 只负责按 narrative_protocol 和通用扩写纪律执行扩写。

#### 输入增加

- `context.md.narrative_protocol`
- `wechat-article-core/references/expansion-methodology.md`（精简执行版）
- `wechat-article-core/references/writing-checklist.md`（精简执行版）
- `wechat-article-core/references/writing-style.md`（通用活人感写作原则）
...
2. **删除第四步"四层自检体系"**
   - 理由：完整质量检查是 `wechat-article-quality` 的职责
...
### 4.7 改 wechat-article-quality

#### 新增输入

- `context.md.narrative_protocol.forbidden_zone`
- `wechat-article-core/references/ai-flavor-guide.md`（完整验收版）
- `wechat-article-core/references/writing-checklist.md`（完整验收版）
- `wechat-article-core/references/writing-style.md`（完整版）
- `context.md.narrative_protocol.derived_from`

#### 职责定位

`wechat-article-quality` 是整个流程中**唯一负责完整四层质量检查**的节点。`khazix-writer` 只输出前轻量禁区扫描，不替代 quality 的职责。
...
### 4.8 改 wechat-article-polish

#### 输入增加

- `context.md.narrative_protocol`
- `wechat-article-core/references/ai-flavor-guide.md`
- `wechat-article-core/references/writing-checklist.md`
- `wechat-article-core/references/writing-style.md`
...
- `wechat-article-core` 目录下增加 `.last-update-check`
- 每 30 天检查一次远程仓库是否有更新
- 本地落后时提示用户更新，不自动执行
...
- `khazix-writer`：参考通用活人感原则，但不绑定具体个人风格
- `wechat-article-polish`：润色时参考
- `wechat-article-quality`：检查通用风格一致性
...
| 文件 | 改动内容 |
|------|---------|
| `wechat-article-core/SKILL.md` | 更新流程，新增 `role_boundary`，删除 `polish_confirmed`，新增 `narrative_protocol` 和 `materials_summary` 字段说明，定义输出目录结构，增加持久化和版本自检 |
| `wechat-article-core/scripts/style_selector.py` | 移除 `max_chars=1200`，完整读取 `.md`，生成 `materials_full.md`，`scan_materials()` 增加 `recursive` 参数 |
| `wechat-article-core/scripts/template_loader.py` | 新增 `extract_narrative_protocol()` 函数 |
| `wechat-article-core/config.yaml` | 新增 `materials.recursive` 和 `materials.extensions` 配置 |
| `wechat-article-core/references/expansion-methodology.md` | 新增：核心原则、材料优先、说话位置、内容骨架、中文句法纪律 |
| `wechat-article-core/references/ai-flavor-guide.md` | 新增：四层 AI 味模型、六种诊断、24 种 AI 写作模式 |
...
| `wechat-article-outline/SKILL.md` | 输入增加 `narrative_protocol`、`expansion-methodology.md`、完整素材；输出 sections 必须按协议生成，并增加 `materials_ref` 和 `human_needed` |
| `khazix-writer/SKILL.md` | 输入增加 `narrative_protocol`、`expansion-methodology.md`、`writing-checklist.md`、`writing-style.md`（精简版）、`outline.sections[].materials_ref`；删除 AI 角色边界、四层自检、卡兹克个人风格绑定；新增"扩写纪律"小节；frontmatter 保留 `narrative_protocol_derived_from` |
...
| `wechat-article-quality/SKILL.md` | 读取 `narrative_protocol.forbidden_zone`、`ai-flavor-guide.md`、`writing-checklist.md`、`writing-style.md`（完整版），增加内容债/语言债诊断，四层检查与方法论对齐 |
| `wechat-article-core/references/failure-handbook.md` | 新增失败降级表（可选） |
...
| khazix-writer 是否保留四层自检 | 否 | 统一由 wechat-article-quality 负责 |
| khazix-writer 是否保留 AI 角色边界说明 | 否 | 统一由 role_boundary 负责 |
| khazix-writer 是否保留卡兹克个人风格绑定 | 否 | 风格由 YAML 模板和 writing-style.md 控制 |
...
6. 更新 `wechat-article-core/SKILL.md` 的 context.md 字段、流程、输出目录、持久化、版本自检。

### 第二阶段：上游子 skill 消费 narrative_protocol

1. 改 `wechat-article-clarify`：从素材收集改为需求确认 + 素材完整性确认，增加说话位置三问。
2. 改 `wechat-article-angle`：基于完整素材和 narrative_protocol 诊断。
3. 改 `wechat-article-outline`：按 narrative_protocol 生成带素材映射的大纲。

### 第三阶段：下游子 skill 消费 narrative_protocol 和扩写方法论

1. 改 `khazix-writer`：按 narrative_protocol 和通用扩写纪律写作，删除卡兹克个人风格绑定。
2. 改 `wechat-article-polish`：按 narrative_protocol、ai-flavor-guide、writing-checklist、writing-style 润色。
3. 改 `wechat-article-title`：按 narrative_protocol.opening 生成标题。
4. 改 `wechat-article-quality`：读取模板禁区，按方法论验收。

### 第四阶段：新增 role_boundary

1. 在 `wechat-article-core` 流程中插入 `role_boundary`。
2. 实现协作契约书输出和确认。
...
- [ ] `wechat-article-clarify` 输出 `requirements.speaker_position`。
- [ ] `wechat-article-angle` 基于完整素材和 `narrative_protocol` 诊断。
- [ ] `wechat-article-outline` 生成的大纲 sections 与 `narrative_protocol` 一一对应。
- [ ] `wechat-article-outline` 的每个 section 包含 `materials_ref` 和 `human_needed`。
- [ ] `khazix-writer` 正文段落能追溯到 `narrative_protocol` 的 section。
- [ ] `khazix-writer` 输出中标注 `ai_filled` 和 `needs_human_experience`。
- [ ] `wechat-article-quality` 能检测模板专属禁区。
- [ ] `wechat-article-quality` 能做内容债/语言债诊断。
