...
- 修复：`references/m1-m9扫描规则.md` 字段名与 gin-fitness-tracker 权威字段元数据对齐——「排便状态」改为「大解状态」（M1/M9，两处），「傍晚情绪」改为「傍晚情绪状态」（M5），并补文件头说明（字段名以「每日记录」表头行运行时读取为准，排便→大解状态、傍晚情绪→傍晚情绪状态、心率→晨起心率（次/分））；M1/M9 的选项表述同步为真实下拉选项（🟢正常1次 / ⚠️异常无/少 / 🔴连续异常），消除分析维度与源表字段对不上的契约漂移
- 优化：`references/main-flow.md` 数据质量闸 1 首次出现处补三必选维度对应的实际字段名（晨起体重、晨起心率（次/分）、大解状态）
- 更新：`SKILL.md` 当前版本行 → v1.4.1
