...
当前场景：gin-fitness-tracker — collect-data（子模块 2/5）
...
④b **问题生成前强制检查清单**（每日轮询/补数据模式）
   ...
   **Cron / 异步触发时**：不等待用户回复，直接返回 `failed` 或 `partial`，记录错误日志；可附带向用户发送离线通知，提示配置异常、本轮轮询跳过。
    ↓
⑤ 根据模式生成问题或解析用户数据
    ↓
...
用户直接报告数据（如"昨天晚上是凌晨1点钟睡的，排便状态有点异常"）。解析数据后：
1. **用 LLM 语义理解从自然语言中提取字段和值**（不要关键词硬匹配）
2. 展示与目标的差异
3. 调用 `write-verify` 写入
4. 处理失败字段
5. 更新用户配置表（体重/体脂）

### 自然语言解析原则

回复录入模式的核心是让 Agent 像人一样理解用户的话，而不是靠关键词脚本硬拆。

**正确做法：**
- 把「字段元数据子表」当作上下文提供给 Agent：字段名、类型、选项、填写说明、时段
- Agent 根据语义识别用户这段话里包含了哪些字段信息
- 对于歧义或不确定的内容，列出可能选项询问用户，不要猜测
- 提取完成后，统一交给 `write-verify` 做类型校验和写入

**示例：**

用户说：
> "昨天晚上是凌晨1点钟睡的。排便状态有点异常。当时也没有感觉到饥饿，入睡难度也正常，中间也没有醒来。"

Agent 应该提取出：
| 字段 | 值 | 推理 |
|------|-----|------|
| 入睡时间 | 01:00 | "凌晨1点钟睡的" → 昨天入睡时间为 01:00 |
| 大解状态 | （需确认） | "有点异常"可对应多个选项，列出让用户选 |
| 起床饥饿程度 | 0 | "没有感觉到饥饿" → 0 分 |
| 入睡难度 | 🟢正常 | "入睡难度也正常" |
| 半夜醒来 | 🟢没有醒来 | "中间也没有醒来" |

**禁止：**
- ❌ 用关键词脚本拆分句子后机械匹配字段
- ❌ 把用户的自然语言原文直接当写入值（尤其单选字段）
- ❌ 对歧义字段擅自替用户做决定

**允许：**
- ✅ Agent 用 LLM 自由理解语义
- ✅ 高置信度提取直接传 `write-verify`
- ✅ 低置信度或歧义时主动追问用户
