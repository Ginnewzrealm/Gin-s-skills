...
进入任意子模块时，先说一句（带 5 子模块定位，让用户知道在整个技能的哪一段）：

```markdown
当前场景：gin-fitness-tracker — [子模块名]（子模块 X/5）
```

例如：
- 初始化 → `当前场景：gin-fitness-tracker — init（子模块 1/5）`
- 数据收集 → `当前场景：gin-fitness-tracker — collect-data（子模块 2/5）`
- 数据查询 → `当前场景：gin-fitness-tracker — query-data（子模块 3/5）`
- 讯记同步 → `当前场景：gin-fitness-tracker — sync-xunji（子模块 4/5）`
- 写入验证 → `当前场景：gin-fitness-tracker — write-verify（子模块 5/5）`
...
| `[硬闸门]` | 用户不确认则不能继续下一步 |
| `[可回环]` | 用户可要求回退到前面步骤重做 |
| `[等待用户回复]` | 生成问题后阻塞等待用户回复才能继续（语义同硬闸门；与 `[自动]` 并列表示该步双态——daily_poll 模式等回复，reply_entry 模式自动解析） |

### 展示时机

1. **流程开始时**：输出场景定位句 + 完整 micro-checklist，高亮当前步骤。
2. **进入硬闸门时**：再次展示 checklist，并追加 `当前阻塞：等待你确认 XXXX。`
3. **过硬闸门/长自动段推进后**：重打 checklist，已完成步标 `[✓]`、当前步高亮 `← 当前`——禁止连续 `[自动]` 步执行期间 checklist 停在初始状态不动（长写入流程中段用子模块状态反馈兼任进度播报）。
4. **完成时**：将最后一步标记为 `[✓]`，输出关键结果（写入字段数、失败字段、未验证项）。
5. **子模块切换时**：输出新子模块的场景定位句 + micro-checklist，继承上游状态。
6. **会话中断恢复时**：读取当前日期行后，重新输出完整 checklist + 当前阻塞提示。

### 禁止

- 不要跳过 `write-verify` 返回的 `needs_user_input` 硬闸门
- 不要静默覆盖用户已有数据
- 不要在用户说"重来"或"回退"时不重置 checklist 状态（回退映射见下表）
- 不要子模块直接向用户输出未经协调的长篇结果

### 回退映射（用户说 X → 回退目标，checklist 同步重置）

| 用户请求 | 回退目标 |
|---|---|
| 「重来」「再试一次」（写入失败后） | 当前子模块 Step 1 重跑；不得绕过 `record_fields_once.py` 完整流程 |
| 「改数据」「填错了」（写入前） | collect-data 重新解析用户数据 |
| 「问题不对」「别问这个」（轮询时） | collect-data 重新生成问题（字段校验重过） |
| 「换日期」「补别的天」 | 路由层重新走 `trigger_classifier.py`，重定 mode/date |
| 「别同步了」（sync 进行中） | 安全中止 sync-xunji；已写入数据不回滚，向用户说明已完成部分 |
