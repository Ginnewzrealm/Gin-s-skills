...
def test_render_mine_shows_current_and_done():
    ...
    assert "- [✓] Step 1 定位/创建项目文件夹 [自动]" in result
    assert "- [ ] Step 2 生成主题定义并确认读者/文体/方向 [硬闸门]  ← 当前" in result
    assert "- [ ] Step 3 拉取锚点素材到成品库 [自动]" in result


def test_render_mine_completed_steps_before_current():
    result = render_mine(current_key="mining")
    assert "- [✓] Step 1 定位/创建项目文件夹 [自动]" in result
    assert "- [✓] Step 2 生成主题定义并确认读者/文体/方向 [硬闸门]" in result
    assert "- [✓] Step 3 拉取锚点素材到成品库 [自动]" in result
    assert "- [✓] Step 4 选择本次挖掘域 [需确认]" in result
    assert "- [ ] Step 5 对话挖掘素材 [硬闸门] [可回环]  ← 当前" in result
