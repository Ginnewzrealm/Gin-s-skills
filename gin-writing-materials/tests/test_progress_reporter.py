...
def test_render_mine_shows_current_and_done():
    ...
    assert "- [✓] Step 1 捕获灵感并创建项目 [自动]" in result
    assert "- [ ] Step 2 形成暂定问题（可回环） [需确认] [可回环]  ← 当前" in result
    assert "- [ ] Step 3 读取可选锚点素材 [自动]" in result


def test_render_mine_completed_steps_before_current():
    result = render_mine(current_key="mining")
    assert "- [✓] Step 1 捕获灵感并创建项目 [自动]" in result
    assert "- [✓] Step 2 形成暂定问题（可回环） [需确认] [可回环]" in result
    assert "- [✓] Step 3 读取可选锚点素材 [自动]" in result
    assert "- [✓] Step 4 确认本次采访方式 [需确认]" in result
    assert "- [ ] Step 5 一次一问地采访并保存素材 [硬闸门] [可回环]  ← 当前" in result
