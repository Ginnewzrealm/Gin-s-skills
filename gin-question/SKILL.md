...
### Step 7：16 格子覆盖度检查（每轮必做，先于饱和判断）

检查 16 个格子（基础 7 + 旅程 5 + 人群场景 2 + 争议时效 2）。**每一轮检索结束后都必须重算覆盖矩阵，覆盖判定永远先于饱和判定。**
...
1. 存在非豁免的 0 覆盖格 → **禁止以"新增率低"为由停止**，必须针对缺失格子发起补搜轮。
2. 补搜轮的做法：列出缺失格 → 用缺失格对应的检索词模板（见「检索词扩展」节）逐格生成查询 → 检索、抽取、归格 → 重算覆盖矩阵。
3. 补搜轮**不计入**饱和连胜统计（即补搜轮新增率低不会触发停止）。
4. 同一格子连续 2 个补搜轮零新增 → 该格子记入 `empty_perspectives` 并注明"补搜 2 轮无新增"，之后允许退出（防无限消耗）。**除此之外，覆盖不全不允许 saturated 退出。**
5. 实战基准：人工会话跑减脂可达 16/16（见 output/test-report.md），自动跑崩不是能力上限，是执行断链——覆盖闭环没走完就 saturated 是典型的执行失败，必须重跑补搜轮。
...
```
1. 覆盖判定（Step 7）：有非豁免缺失格？
   → 有：必须发起补搜轮，回到第 1 步（补搜轮不计入饱和连胜）
   → 无：进入第 2 步
2. 饱和判定：
   新增率 = 本轮新增问题数 / 上轮总问题数
   连续 3 轮新增率 < 50%（只统计非补搜轮）→ 停止
3. 例外放行：某格子已连续 2 个补搜轮零新增（已记 empty_perspectives）
   → 该格不再阻塞饱和判定
```
...
- **停止前最终自检**：exit_reason 写 `saturated` 之前，必须确认 coverage 无非豁免缺失格（或均已按第 3 条放行）——覆盖不全 + saturated 是非法输出，必须补搜后重判。
...
```json
{
  ...
  "empty_perspectives": [],
  "rounds": [
    {"round": 1, "new_count": 42, "new_rate": 1.0, "backfill": false, "target_cells": []},
    {"round": 3, "new_count": 3, "new_rate": 0.04, "backfill": true, "target_cells": ["旅程/执行期", "旅程/瓶颈期"]}
  ]
}
```

`rounds` 逐轮记录新增数/新增率/是否补搜轮/补搜目标格——覆盖崩溃类问题靠此字段定位，禁止省略。
...
| 覆盖度未全 | 0 | 按 Step 7 补搜闭环执行：非豁免缺失格必须补搜（不计入饱和连胜）；仅"同格补搜 2 轮零新增"可记 empty_perspectives 放行；覆盖不全 + saturated 退出 = 非法输出 |
