...
description: xie-jian-li 的 STAR 隐性知识挖掘方法论，改编自 tacit-mining。
type: reference
---

# xie-jian-li —— STAR 隐性知识挖掘方法论
...
```
[提问] → 用户回答 → [可选追问 1-2 次] → [Teachback] → 用户确认/纠正 → [存碎片] → 下一轮
```

**Teachback 格式**：

```
"所以当时的情况是 [S]，你的任务是 [T]，你做了 [A]，最后 [R]。对吗？"
```

用户确认后写入碎片；用户纠正后使用纠正版本。
