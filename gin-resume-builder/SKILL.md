...
> 当前版本：v1.26.0（2026-09-19）· 变更记录见技能目录「更新日志.md」
...
2. **知识库检查**：读取技能目录 `config.yaml` 的 `kb_path`：
   ...
   - 结构完整 → **先跑 `python3 scripts/kb_freshness.py --kb <kb路径>`**（facts.yaml 新鲜度检查）：`原始事实/` 任一文件比 facts.yaml 新 = 索引过期 → 自动重建后再继续。**禁止在索引过期状态下直接查询**——按读取纪律查旧索引会漏，Agent 查漏后退回全库乱翻，纪律白写
   - 新鲜度确认 → 按路由目标执行
...
知识库四层目录有严格的真源分工，**读取必须走指定入口，禁止全库无差别搜索**（裸 grep 会同时命中派生物的旧拷贝，把漂移当成事实）：

| 问题类型 | 唯一读取入口 | 禁止 |
|---|---|---|
| 事实类问题（「我之前X项目怎么写的」「开单细节是什么」） | `自动生成/facts.yaml` + `原始事实/*.md` | 搜 `面试素材/`、`生成物/` |
| 面试表达类（STAR 故事怎么讲） | `面试素材/star_stories.md` | 从生成物简历里翻故事 |
| 已生成成品（旧简历/报告） | `生成物/` 对应子目录 | 把生成物当事实来源引用 |

三条硬规则：

1. **派生物禁止作为事实来源**：`面试素材/` 与 `生成物/` 是派生层，内容不得作为回答用户事实问题的依据——它们可能含有未回流的旧措辞，引用即把漂移固化进新产物。
2. **查无即告缺**：facts.yaml 与 `原始事实/` 都查不到 → 如实告知「知识库无此记录」，提示走「增量更新」录入；**禁止**退而求其次去生成物里翻凑答案。
3. **发现漂移即报告**：任何环节发现派生物里有而源里没有的细节 → 运行 `python3 scripts/kb_drift_check.py --kb <kb路径>` 生成待回流清单，请用户确认后回写 `原始事实/`，**不静默引用、不静默吸收**。
...
| 盘点技能 / 录入技能 / 技能清单 / 我会哪些技能 / 挖掘能力 | 技能盘点 | 先读 `references/skill-inventory-methodology.md` 与 `references/skills-inventory-standard.md`；确认后写入 |
...
| 我之前XX怎么写的 / XX细节是什么（职业事实自由问答） | 事实问答 | 读取纪律见上文：只读 facts.yaml + 原始事实/，查无即告缺，禁止搜派生层 |
...
**Progress：**
- [ ] Step 1 确认访谈目标（初始化 / 增量更新 / 技能盘点 / 技能深挖）`[需确认]`
...
- **技能盘点**：先读 `references/skill-inventory-methodology.md`，建立目标卡，按「概念拆解 → 动作还原 → 决策与结果 → 技能提炼 → 证据确认」推进；能力、经验和资源线索作为候选对象分别归类，不把资源或一次性成果直接写成技能。
...
- **技能深挖**：为核心技能写详细描述前信息不足时，按 `references/skill-mining-playbook.md` 支架式提问（把创作题变成选择题/填空题/改错题，不抛开放式大问题），核心技能挖全四维详情并做反事实校验、其余从简；产物经用户确认后写入 `原始事实/skill_details.md` 并运行 `facts_parser.py` 重建
...
| 产物 | 生成时机 | 内容格式 | 给谁用 |
|---|---|---|---|
| `skill_details.md` | 用户主动要求「深挖某技能」时，用 `skill-mining-playbook.md` 支架式提问 | 四维详情 + 反事实校验：情境/行动/结果/沉淀 | 简历岗位胜任、面试 STAR 故事 |
...
5. **事实挑选**：`fact_selector.py --jd <jd> --json-out picked.json --matrix-out matrix.json`（转行加 --career-switch），默认不限制 bullet 数量，按 JD 相关度排序后展示全部候选，并展示 JD—证据匹配矩阵，由用户在渲染前确认环节手动删减
...
- 冷邮件场景不联网调研（仅使用用户信息+知识库）；求职信/面试清单场景允许联网调研目标公司，但结果必须写入 `company_research/` 缓存复用
- 不代投简历、不代发邮件、不登录招聘平台
