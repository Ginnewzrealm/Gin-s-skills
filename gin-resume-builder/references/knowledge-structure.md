...
│   ├── basic_info.md          # - 键: 值（姓名/电话/邮箱/城市/求职意向/年龄/教育背景）
│   ├── work_history.md        # ## 公司 | 职位 | 2020.01-2023.05 + bullet 列表
│   │                          #   特殊条目：## 职业休整期 | 休整期类型 | 起止时间，用于填充工作经历时间线断档
...
└── 生成物/                    # 可清理临时产物，删除不影响知识库
    ...
    └── 简历/                  # ⚠️ 旧版残留目录（规范外）：存量文件迁移至 resumes/ 后删除，
                               #   游离在规范外的目录是信息漂移的温床
```

## 真源纪律：写入回流与读取入口

分层设计成立的前提是「源全、派生物纯」。三条铁律：

1. **派生物禁止首发信息**：任何环节（简历打磨/面试准备/JD 分析）产生的新事实或细节，
   **确认后必须先回写 `原始事实/` 对应文件、重跑 `facts_parser.py` 重建 facts.yaml，
   之后才允许出现在任何派生物里**。派生物里只许出现源里已有的内容（允许改写，不允许新增）。
2. **读取走指定入口，禁止全库裸搜**：事实类问题只读 `原始事实/` + `自动生成/facts.yaml`；
   `面试素材/` 与 `生成物/` 不得作为事实来源（格式分工表见 SKILL.md「读取纪律」）。
   理由：裸 grep 会同时命中派生物的旧拷贝，把漂移当成事实固化进新产物。
3. **漂移巡检**：`python3 scripts/kb_drift_check.py --kb <路径>` 对比派生层与源的关键词覆盖，
   输出「派生物有、源没有」的待回流清单。发现漂移 → 请用户确认后回写源，不静默引用。
