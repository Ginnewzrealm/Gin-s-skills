...
def _resolve_tokens_file():
    """凭据文件路径：环境变量 > 技能目录 config/tokens.json > 旧版 wexin-read-mcp 路径（兼容回退）"""
    env_path = os.environ.get('GETNOTE_TOKENS_FILE')
    if env_path:
        return os.path.abspath(os.path.expanduser(env_path))
    skill_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    independent = os.path.join(skill_root, 'config', 'tokens.json')
    if os.path.exists(independent):
        return independent
    legacy = os.path.join(skill_root, 'wexin-read-mcp', 'tokens.json')
