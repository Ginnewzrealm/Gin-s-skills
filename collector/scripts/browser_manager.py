#!/usr/bin/env python3
"""
OpenCLI 浏览器生命周期管理器（collector 专用）。

职责：
1. 首次初始化（mode=init）：检测 opencli、Chrome、profile，保存配置
2. 每次使用（mode=use）：确保浏览器运行在目标 profile 且扩展已连接
3. 清理（mode=cleanup）：释放 browser session 并清理残留窗口

核心设计：
- Chrome 未运行 → 用目标 profile 启动
- Chrome 在运行但目标 profile 未连接 → 强制退出并重启到目标 profile
- 目标 profile 已连接 → 直接成功
- 所有浏览器操作前获取文件锁，防止多 Agent 并发冲突

本脚本由 collector/SKILL.md 规范，被 main.py 在调用 opencli 命令前后使用。
"""
...
WORKSPACE_DIR = os.path.abspath(os.path.join(_SCRIPT_DIR, "..", "workspace"))
LOCK_FILE_PATH = os.path.join(WORKSPACE_DIR, ".browser_manager.lock")

# 复用 config_manager 的配置读写，避免双源维护
try:
...
class BrowserManager:
    ...
    def _acquire_lock(self, timeout: int = 30) -> bool:
        ...
        try:
            ...
            print(f"[BrowserManager] 创建锁文件失败: {e}")
            return False
    ...
    def _release_lock(self) -> None:
        ...
        try:
            ...
            print(f"[BrowserManager] 释放锁文件失败: {e}")
    ...
    def _run_init(self) -> Dict[str, Any]:
        ...
        self.config["opencli"]["installed"] = version is not None
        self.config["opencli"]["version"] = version
        self.config["env_check"]["checks"]["opencli_installed"] = version is not None
        ...
        if not version:
            return self._error(
                ...
                impact="无法使用 OpenCLI 采集。",
            )

        # 2. 检查 doctor
        doctor_ok = self._run_doctor()
        self.config["opencli"]["doctor_passed"] = doctor_ok
        self.config["env_check"]["checks"]["doctor_passed"] = doctor_ok
        save_config(self.config)
        ...
        self.config["env_check"]["checks"]["chrome_available"] = chrome_available
        save_config(self.config)
        ...
        # 4. 读取 Chrome 本地 profile 列表
        chrome_profiles = self._get_chrome_profiles()

        # 5. 获取 OpenCLI profile 列表
        ...
        # 6. 建立 Chrome profile 与 OpenCLI profile 的对应关系
        ...
        self.config["browser_profile"]["chrome_profile_name"] = selected.get("chrome_name", "Unknown")
        self.config["env_check"]["checks"]["profile_selected"] = True
        save_config(self.config)

        # 8. 切换默认 profile
        profile_id = self.config["browser_profile"]["opencli_profile_id"]
        chrome_profile_id = self.config["browser_profile"]["chrome_profile_id"]
        if profile_id:
        ...
        self.config["env_check"]["initialized_at"] = datetime.now(timezone.utc).isoformat()
        save_config(self.config)
        ...
        return {
            ...
            "message": f"✅ 浏览器环境初始化完成。OpenCLI {version}，Chrome profile '{chrome_name}'（{profile_id}）已连接。",
            "data": {
                "initialized": True,
    ...
    def _select_profile(
        ...
        优先级：
        1. 单 profile → 自动使用
        2. 多 profile → 优先选择 email/名称包含 openclaw 或 opencli 的
        3. 否则选择 default 标记的
        4. 否则选择第一个
        """
    ...
    def _run_use(self, session_name: Optional[str]) -> Dict[str, Any]:
        """每次使用前的浏览器就绪流程。"""
        self.config = load_config()
        session = session_name or self.config["browser"].get("session_name", "collector")
        ...
        try:
            ...
            # 避免多个分组累积；只关标题匹配的标签，不影响用户正常窗口
            print("[BrowserManager] 前置清理：关闭已有 OpenCLI Browser 残留标签...")
            ...
            if not ready_result.get("success"):
                ...
                message_map = {
                    "CHROME_LAUNCH_FAILED": ("无法启动 Chrome。", "open -a / google-chrome 执行失败。"),
                    ...
                    "EXTENSION_NOT_INSTALLED": ("OpenCLI 扩展未在目标 Chrome profile 中连接。", "Chrome 已确保运行在目标 profile，但扩展仍未连接。"),
                    "CHROME_QUIT_FAILED": ("无法退出当前 Chrome。", "osascript / pkill / taskkill 执行失败。"),
                    "CHROME_EXIT_TIMEOUT": ("等待 Chrome 退出超时。", "Chrome 未在预期时间内完全关闭。"),
                }
                short, reason = message_map.get(code, ("浏览器就绪失败。", ready_result.get("reason", "未知原因")))
                return self._error(
                    code,
                    short,
                    reason=reason,
    ...
    def _run_cleanup(self, session_name: Optional[str]) -> Dict[str, Any]:
        ...
        # 3. 智能判断：若 Chrome 由本技能启动且当前只剩残留窗口，则完全退出 Chrome
        if self.browser_opened_by_skill:
            try:
                if self._is_chrome_running():
                    if self._only_leaked_windows():
                        if self.quit_chrome():
                            results.append("本技能启动的 Chrome 已无正常窗口，已完全退出 Chrome 进程")
                        else:
                            msg = "退出 Chrome 进程失败"
                            results.append(msg)
                            errors.append({"code": "CLEANUP_WARNING", "message": msg})
                    else:
                        results.append("检测到用户正常窗口，保留 Chrome 进程")
                else:
                    results.append("Chrome 已不在运行，无需退出")
            except Exception as e:
                msg = f"智能退出 Chrome 进程异常: {e}"
                results.append(msg)
                errors.append({"code": "CLEANUP_EXCEPTION", "message": msg})
        ...
        return {
            ...
            "module": "browser-manager",
            "message": (("✅ 浏览器清理完成。" if not errors else "⚠️ 浏览器清理部分失败。") + "；".join(results)),
            "data": {},
            "errors": errors
        }
    ...
    def ensure_browser_with_profile(
        ...
        返回：
            {"success": True} 或 {"success": False, "code": "...", "reason": "..."}
        """
        # 1. 快速路径：目标已连接
        connected_id = self.get_connected_opencli_profile()
        if connected_id == opencli_profile_id and self._run_doctor():
            print(f"[BrowserManager] 目标 profile {opencli_profile_id} 已连接，跳过浏览器操作")
            return {"success": True}
        ...
        if chrome_running:
            if connected_id is None:
                print("[BrowserManager] Chrome 在运行但无 OpenCLI profile 连接，准备重启到目标 profile")
            else:
                print(f"[BrowserManager] Chrome 在运行但当前 profile {connected_id} 不是目标 {opencli_profile_id}，准备重启 Chrome")
            ...
            if not self.wait_for_chrome_exit(max_attempts=60, interval=0.5):
                return {"success": False, "code": "CHROME_EXIT_TIMEOUT", "reason": "等待 Chrome 退出超时"}

        # 3. 用目标 profile 启动 Chrome
        print(f"[BrowserManager] 用 profile {chrome_profile_dir} 启动 Chrome")
        ...
        if not self._wait_for_chrome(max_wait=15):
            return {"success": False, "code": "CHROME_LAUNCH_TIMEOUT", "reason": "Chrome 启动后未检测到进程"}

        # 5. 切换 OpenCLI profile
        print(f"[BrowserManager] 切换 OpenCLI profile: {opencli_profile_id}")
        if not self._switch_profile(opencli_profile_id):
            return {"success": False, "code": "PROFILE_SWITCH_FAILED", "reason": "opencli profile use 执行失败"}

        # 6. 轮询等待扩展连接
        print("[BrowserManager] 等待 extension 连接...")
        for i in range(max_retry):
            connected_id = self.get_connected_opencli_profile()
            if connected_id == opencli_profile_id and self._run_doctor():
                print(f"[BrowserManager] extension 已连接 ({opencli_profile_id})")
                self.browser_opened_by_skill = True
                return {"success": True}
            print(f"[BrowserManager] 等待 extension 连接... ({i + 1}/{max_retry})")
            time.sleep(retry_interval)

        return {"success": False, "code": "EXTENSION_NOT_INSTALLED", "reason": "扩展连接超时"}

    def get_connected_opencli_profile(self) -> Optional[str]:
        """
        获取当前已连接的 OpenCLI profile ID。

        解析 `opencli profile list` 输出，返回 connected 状态的 profile ID。
        未连接时返回 None。
        """
        try:
            result = subprocess.run(
            ...
            print(f"[BrowserManager] 获取已连接 profile 失败: {e}")
            return None
    ...
    def launch_chrome(self, profile_dir: Optional[str] = None) -> bool:
        ...
        参数：
            profile_dir: Chrome profile 目录名，如 "Profile 1"
        """
        ...
        try:
            if system == "Darwin":
                ...
                chrome_cmd = ["google-chrome", "--new-window", "about:blank"]
                if profile_dir:
                ...
                chrome_cmd = ["start", "chrome", "--new-window", "about:blank"]
                if profile_dir:
                ...
                print(f"[BrowserManager] 不支持的平台: {system}")
                return False

        except Exception as e:
            print(f"[BrowserManager] 启动 Chrome 失败: {e}")
            return False
    ...
    def quit_chrome(self) -> bool:
        ...
        try:
            ...
            if self._is_chrome_running():
                if system == "Darwin":
                    subprocess.run(["pkill", "-x", "Google Chrome"], check=False, timeout=10)
                elif system == "Linux":
            ...
            print(f"[BrowserManager] 退出 Chrome 失败: {e}")
            return False
    ...
    def wait_for_chrome_exit(self, max_attempts: int = 60, interval: float = 0.5) -> bool:
        ...
        for i in range(max_attempts):
            if not self._is_chrome_running():
                return True
            if i % 4 == 0:
                print(f"[BrowserManager] 等待 Chrome 退出... ({i + 1}/{max_attempts})")
            time.sleep(interval)
        return False
    ...
    def cleanup_leaked_windows(self) -> str:
        ...
        2. 关闭所有标签都是 about:blank / chrome://newtab / OpenCLI Browser 的窗口
        不会误关用户的正常标签。
        """
    ...
    def _only_leaked_windows(self) -> bool:
        """
        判断当前 Chrome 是否只剩残留窗口（可安全退出）。

        返回 True：所有窗口的所有标签都是 about:blank / chrome://newtab / OpenCLI Browser
        返回 False：存在至少一个正常窗口
        """
        if platform.system() != "Darwin":
            # 非 macOS 无法精确判断窗口内容，保守返回 False，不自动退出
            return False
        ...
        tell application "Google Chrome"
            ...
            repeat with w from 1 to (count windows)
                ...
                repeat with t in tabs of win
                    set tabURL to URL of t
                    set tabTitle to title of t
                    if not (tabURL starts with "about:blank" or tabURL starts with "chrome://newtab" or tabTitle contains "OpenCLI Browser") then
                        return "false"
                    end if
                end repeat
            end repeat
        ...
        try:
            ...
            print(f"[BrowserManager] 判断窗口内容失败: {e}")
            return False

    # ---------- 底层命令 ----------

    def _get_opencli_version(self) -> Optional[str]:
        """获取 opencli 版本，未安装返回 None。"""
    ...
    def _run_doctor(self) -> bool:
        ...
        opencli doctor 退出码总是 0，不能用 returncode 判断。
        通过条件：无 [FAIL] 项且 Connectivity 未失败。
        """
    ...
    def _bridge_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode != 0 or has_fail or has_missing_extension:
                ...
                if "Extension" in output and ("not connected" in output or "MISSING" in output):
    ...
    def _parse_profile_list(self, output: str) -> List[Dict[str, str]]:
        ...
        for line in output.splitlines():
            ...
            name = ""
            is_connected = in_connected or ("connected" in line.lower() and "not connected" not in line.lower())

            for part in parts[1:]:
                lower = part.lower()
                if lower in ("connected", "not", "—", "--", "default", "v1.0.22"):
                    continue
    ...
    def _get_chrome_profiles(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            print(f"[BrowserManager] 读取 Chrome Local State 失败: {e}")
            return []
    ...
    def _get_current_chrome_profile_dir(self) -> Optional[str]:
        ...
        这是确定 OpenCLI profile ID 对应哪个 Chrome profile 目录的最可靠方法。
        """
        system = platform.system()
        try:
            if system == "Darwin":
            ...
            print(f"[BrowserManager] 解析当前 Chrome profile 失败: {e}")
        return None
    ...
    def _match_profiles(
        ...
        匹配规则（按优先级）：
        1. 扩展已连接时，根据当前运行的 Chrome 进程命令行中的 --profile-directory 直接匹配
        2. OpenCLI profile 的 name 与 Chrome profile 的 id 或 name 相同（忽略大小写）
        3. 无法匹配 → 按顺序逐一对应
        """
        matched = []
        used_chrome_ids = set()
    ...
    def _error(self, code: str, short: str, reason: str = "", action: str = "", impact: str = "") -> Dict[str, Any]:
        """构造统一错误返回。"""
        message = f"❌ {short}"
        if reason:
            message += f"\n原因：{reason}"
...
def main():
    ...
    result = manager.run(mode, session)
